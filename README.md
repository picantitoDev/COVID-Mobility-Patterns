...
7. [Storage and Modeling Results](#️-storage-and-modeling-results)
...
# BigQuery Datasets (Do not change)
KESTRA_GCP_DATASET_STAGING=staging
KESTRA_GCP_DATASET_INTERMEDIATE=intermediate
KESTRA_GCP_DATASET_MARTS=marts
```

⚠️ Note: Do not change the default dataset names (staging, intermediate, marts) in the .env file. BigQuery dataset names are hardcoded in both Terraform (main.tf) and dbt (dbt_project.yml). They follow a fixed layered architecture (staging, intermediate, marts). Changing them requires coordinated updates across both infrastructure and transformation layers.
